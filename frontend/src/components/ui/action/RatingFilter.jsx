  ...
  return (
    <div className="flex items-center gap-2 relative">
      <input
        type="checkbox"
        id={`rating-${num}`}
        className="accent-primary w-8 h-8 absolute opacity-0 z-[999999]"
        onChange={() => onChange(num)}
        checked={value?.includes(num)}
      />
      <div className={`w-4 h-4 border border-solid flex items-center justify-center rounded-full ${!value?.includes(num) ? 'border-gray-400' : 'border-primary'} `}>
